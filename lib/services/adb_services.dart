...
class ADBService {
  ...
  Future<List<Item>> getDirectoryContents(String currentPath) async {
    ...
    for (int i = 0; i < directoryContentDetails.length; i++) {
      if (directoryContentDetails[i] == "./" ||
          directoryContentDetails[i] == "../") {
        continue;
      }
      directoryItems.add(Item(
          directoryContentDetails[i].endsWith("/")
              ? directoryContentDetails[i].replaceAll("/", "")
              : directoryContentDetails[i],
          getFileType(
              device: device,
              currentPath: currentPath,
              fileName: directoryContentDetails[i])));
    }
  ...
  Future<List<Storage>> getExternalStorages() async {
    ...
    for (int i = 0; i < storages.length; i++) {
      if (storages[i].itemName != "self" &&
          storages[i].itemName != "emulated" &&
          storages[i].itemName != "." &&
          storages[i].itemName != "..") {
        externalStorages.add(
            Storage("/storage/${storages[i].itemName}/", storages[i].itemName));
      }
    }
  ...
  Future<int> deleteItem(
    ...
    ProcessResult result = await Process.run(
        adbExecutable, ["-s", device.id, "shell", "rm", "-r", "\"$itemPath\""]);
  ...
  void uploadContent(
      ...
      required FileItemType uploadType,
      Function? onProgress}) async {
    //Send file/directory to Android device using adb push
    String? sourcePath = await pickFileFolderFromDesktop(
        fileItemType: uploadType,
        dialogTitle: uploadType == FileItemType.file
            ? "Select File"
            : "Select Directory",
        allowedExtensions: ["*"]);
    ...
    if (onProgress != null) {
      onProgress(process, getDesktopFileSize, getFileItemSize, sourcePath,
          currentPath);
    }
  }
  ...
  void downloadContent({required String itemPath, Function? onProgress}) async {
    String? chosenDirectory = await pickFileFolderFromDesktop(
        fileItemType: FileItemType.directory,
        dialogTitle: "Where to download",
        allowedExtensions: ["*"]);
    ...
    Process process = await Process.start(
        adbExecutable, ["-s", device.id, "pull", itemPath, chosenDirectory]);
    if (onProgress != null) {
      onProgress(process, getFileItemSize, getDesktopFileSize, itemPath,
          chosenDirectory);
    }
  }
  ...
  Future<int> getFileItemSize(String filePath) async {
    ProcessResult processResult = await Process.run(
        adbExecutable, ["-s", device.id, "shell", "du", "-s", "\"$filePath\""]);
  ...
  Future<List<Map<String, dynamic>>> getAppPackageInfo(AppType appType) async {
    ...
    if (packageInfoList.length == 1) {
      // empty line returned, maybe error occurred
      arguments.add("--user 0");
      result = await Process.run(adbExecutable, arguments);
      packageInfoList = result.stdout.toString().split("\n");
    }
  ...
  Future<int> getAppPackages(String packageName, String chosenDirectory) async {
    ...
    ProcessResult result = await Process.run(
        adbExecutable, ["-s", device.id, "shell", "pm", "path", packageName]);
    ...
    for (int i = 0; i < paths.length; i++) {
      ...
      await Process.run(
          adbExecutable, ["-s", device.id, "pull", paths[i], chosenDirectory]);
  ...
  Future<List<String>> getUninstalledSystemApps() async {
    ProcessResult result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "pm", "list", "packages", "-s"]);
    ...
    result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "pm", "list", "packages", "-u", "-s"]);
    List<String> systemAppsIncludingUninstalled =
        result.stdout.toString().split("\n");
    systemAppsIncludingUninstalled.removeLast();
    for (int i = 0; i < systemAppsIncludingUninstalled.length; i++) {
      systemAppsIncludingUninstalled[i] =
          systemAppsIncludingUninstalled[i].trim();
    }
    ...
    for (int i = 0; i < systemAppsIncludingUninstalled.length; i++) {
      if (!systemApps.contains(systemAppsIncludingUninstalled[i])) {
        uninstalledSystemApps
            .add(systemAppsIncludingUninstalled[i].split(":")[1]);
      }
    }
  ...
  Future<Process> reinstallSystemApp(String packageName) async {
    return await Process.start(adbExecutable,
        ["-s", device.id, "shell", "pm", "install-existing", packageName]);
  }

  Future<void> forceStopPackage(String packageName) async {
    await Process.run(adbExecutable,
        ["-s", device.id, "shell", "am", "force-stop", packageName]);
  }

  Future<int> uninstallApp(
      {required String packageName, bool keepData = false}) async {
    ProcessResult result;
    if (keepData) {
      result = await Process.run(adbExecutable,
          ["-s", device.id, "shell", "pm", "uninstall", "-k", packageName]);
    } else {
      result = await Process.run(
          adbExecutable, ["-s", device.id, "uninstall", packageName]);
    }
    return result.exitCode;
  }
  ...
  Future<int> launchApp({required String packageName}) async {
    ProcessResult result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "monkey", "-p", packageName, "1"]);
    return result.exitCode;
  }

  Future<Process> reinstallSystemAppForUser(
  ...
  Future<Process> installSingleApk(String apkFilePath) async {
    ...
    return await Process.start(
        adbExecutable, ["-s", device.id, "install", apkFilePath]);
  }

  Future<Process> installMultipleForSinglePackage(
      List<String> apkFilePaths) async {
  ...
  Future<int> suspendApp(String packageName) async {
    ...
    ProcessResult result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "pm", "suspend", packageName]);
    return result.exitCode;
  }
  ...
  Future<int> unsuspendApp(String packageName) async {
    ProcessResult result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "pm", "unsuspend", packageName]);
    return result.exitCode;
  }

  Future<int> compileApp(
      String packageName, CompilationMode compilationMode) async {
    ProcessResult result = await Process.run(adbExecutable, [
      "-s",
      device.id,
      "shell",
      ...
      "compile",
      "-m",
      getCompilationModeAsString(compilationMode),
      "-f",
      packageName
    ]);
    return result.exitCode;
  ...
  Future<int> excludeFromMediaScanner(String path) async {
    ...
    ProcessResult result = await Process.run(adbExecutable,
        ["-s", device.id, "shell", "touch", "\"$path.nomedia\""]);
    return result.exitCode;
  }
  ...
  Future<String> getDeviceProperty(String property) async {
    try {
      ProcessResult result = await Process.run(
          adbExecutable, ["-s", device.id, "shell", "getprop", property]);
  ...
  Future<bool> amIRoot() async {
    try {
      ProcessResult result = await Process.run(
          adbExecutable, ["-s", device.id, "shell", "whoami"]);
  ...
  Future<bool> obtainRoot() async {
    try {
      ProcessResult result =
          await Process.run(adbExecutable, ["-s", device.id, "root"]);
